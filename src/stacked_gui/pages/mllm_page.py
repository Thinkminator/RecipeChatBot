...
class MLLMPage(BasePage):
    def __init__(self, parent, app):
        ...
        self.title_var.set("MLLM Interface")
        ...
        # Prompt text input without placeholder text
        self.prompt_var = tk.StringVar()
        self.prompt_entry = ttk.Entry(row, textvariable=self.prompt_var, font=("Segoe UI", 12))
        self.prompt_entry.pack(side="left", fill="both", expand=True)

        button_frame = ttk.Frame(row)
        button_frame.pack(side="right")

        self.choose_btn = ttk.Button(button_frame, text="📁 Choose Image", command=self.open_image)
    ...
    def open_image(self):
        ...
        if file_path:
            ...
            # Disable choose button and change text to "Selected"
            self.choose_btn.config(state="disabled", text="Selected")
        else:
            self.selected_image_path = None
            self.send_btn.config(state="disabled")
            # Enable choose button and reset text
            self.choose_btn.config(state="normal", text="📁 Choose Image")

    def insert_image_in_chat(self, image_path, prompt_text=None):
        try:
            ...
            # Insert prompt text if provided
            if prompt_text:
                self.chat.insert(tk.END, f"You: {prompt_text}\n", "user")
            else:
    ...
    def on_send(self):
        ...
        # Insert combined user prompt and image
        self.insert_image_in_chat(self.selected_image_path, prompt_text)
        ...
        def worker():
            try:
                ...
                caption = infer_dish_from_image(self.selected_image_path, prompt=prompt_text)
                if not caption:
                    caption = "an unknown dish"

                combined_prompt = f"{caption}. {prompt_text}"

                mode = self.app.state.get("mode")

                response = generate_bot_reply(mode, combined_prompt, app_state=self.app.state, image_path=self.selected_image_path)

                self.chat.config(state="normal")
                self.chat.insert(tk.END, f"Bot (image understanding): I think this is: {caption}\n", "bot")
                clean_response = self.truncate_at_user(response)
                self.chat.insert(tk.END, f"{clean_response}\n", "bot")
                self.chat.config(state="disabled")
                self.chat.yview(tk.END)
    ...
    def on_show(self, **_):
        ...
        self.title_var.set(f"MLLM Interface — {mode.replace('_', ' ').title()}")

    def on_clear(self):
        # stop any ongoing speech and reset the Speak button
        if hasattr(self, 'speaking') and self.speaking and self.engine:
            self.engine.stop()
        ...
        self.speak_btn.last_reply = None

    def truncate_at_user(self, text):
        idx = text.find("User:")
        if idx == -1:
            return text.strip()
